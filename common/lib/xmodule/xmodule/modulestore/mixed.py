...
class MixedModuleStore(ModuleStoreWriteBase):
    ...
    def _incoming_reference_adaptor(self, store, course_id, reference):
        ...
        """
        if isinstance(reference, store.reference_type):
            return reference
        if store.reference_type == Location:
            return loc_mapper().translate_locator_to_location(reference)
        return loc_mapper().translate_location(course_id, reference, reference.revision == 'draft', True)

    def _outgoing_reference_adaptor(self, course_id, reference):
        """
    ...
    def _incoming_xblock_adaptor(self, store, course_id, xblock):
        ...
        """
        # TODO surgery on jump-to, jump-to-module, and any other rewritten url types
        # see xmodule.modulestore.xml_importer.import_module.rewrite_nonportable_content_links
        for field in xblock.fields:
            if isinstance(field, Reference) and field.is_set_on(xblock):
                field.write_to(
                    xblock,
                    self._incoming_reference_adaptor(store, course_id, field.read_from(xblock)))
            elif isinstance(field, ReferenceList) and field.is_set_on(xblock):
                field.write_to(
                    xblock,
    ...
    def _outgoing_xblock_adaptor(self, course_id, xblock):
        """
        Change all reference fields in this xblock to the type expected by the persistence layer
        """
        # TODO surgery on jump-to, jump-to-module, and any other rewritten url types
        for field in xblock.fields:
            if isinstance(field, Reference) and field.is_set_on(xblock):
                field.write_to(
                    xblock,
                    self._outgoing_reference_adaptor(course_id, field.read_from(xblock)))
            elif isinstance(field, ReferenceList) and field.is_set_on(xblock):
                field.write_to(
                    xblock,
                    [self._outgoing_reference_adaptor(course_id, ref)
                     for ref in field.read_from(xblock)]
                )
    ...
    def get_instance(self, course_id, location, depth=0):
        ...
        return self._outgoing_xblock_adaptor(course_id, xblock)
    ...
    def get_items(self, location, course_id=None, depth=0, qualifiers=None):
        ...
        xblocks = [self._outgoing_xblock_adaptor(course_id, xblock) for xblock in xblocks]
        return xblocks
    ...
    def get_course(self, course_id):
        ...
        if store.reference_type == Location:
            ...
            if isinstance(course_id, CourseLocator) or '/' not in course_id:
    ...
    def get_parent_locations(self, location, course_id):
        ...
        return [self._outgoing_reference_adaptor(course_id, reference)
